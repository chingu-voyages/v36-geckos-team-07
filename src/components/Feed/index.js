...
class MainFeed extends Component {
  ...
  render() {
    ...
    const filteredNews = news.filter((news) =>
      news.name.toLowerCase().includes(dropdown.toLowerCase())
    );
          ...
          <div className="daily-news">
            ...
            <div className="page-head-container" style={{height: "80px"}}>
              <div>
                <h1>News Source</h1>
                <p>
                  Your Daily news source for all things crypto, blockchain, and
                  NFTs
                </p>
              </div>
              <DropDown formLabel="Choose news source" onChange={handleSelect}>
                {
                  // maps through crypto news api and pushes the news api names to the created empty nameArray
                  news.map((news) => nameArray.push(news.name))
                }
                {
